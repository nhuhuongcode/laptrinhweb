...
<ul class="product_list">
    <?php
        ...
        taoKetNoi($link);
        $from_p = 0;
                if(isset($_POST['keyword'])&& isset($_GET['page']))
                    $keyword = $_POST['keyword'];
                    $from = NUM_PROD_PER_PAGE * $from_p;
        
                    $result = chayTruyVanTraVeDL($link, "select * from tbl_sach 
                                                        inner join tbl_tac_gia on tbl_sach.id_tac_gia = tbl_tac_gia.id_tac_gia 
                                                        where ten_sach like '%".$keyword."%' limit ".$from.",".NUM_PROD_PER_PAGE.";");
...
<ul class="pagination">
    <?php
            $result2 = chayTruyVanTraVeDL($link, "select count(*) from tbl_sach 
            inner join tbl_tac_gia on tbl_sach.id_tac_gia = tbl_tac_gia.id_tac_gia 
            where ten_sach like '%".$keyword."%';");
            $total_r = mysqli_fetch_row($result2);
            $total = $total_r[0];
